    ...
    if (TRX_ADDONS_STORAGE['scroll_to_anchor'] == 1) {
        ...
        jQuery(window).bind('mousewheel DOMMouseScroll', function(e) {
            ...
            if (wheel_dir == -1) {
                doit = true;
                setTimeout(function() {
                    if (items.prev >= 0 && items.prevOffset >= scroll_offset - wh - ah) toc_menu_items.eq(items.prev).find('a').trigger('click');
                    else trx_addons_document_animate_to(Math.max(0, scroll_offset - wh));
                }, 10);
            } else {
                doit = true;
                setTimeout(function() {
                    if (items.next >= 0 && items.nextOffset <= scroll_offset + wh + ah) toc_menu_items.eq(items.next).find('a').trigger('click');
